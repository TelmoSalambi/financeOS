      ...
      <div className="max-w-6xl mx-auto animate-in fade-in duration-700">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          <div>
            ...
            <p className="text-slate-500 mt-1">Análise inteligente da sua saúde financeira.</p>
          </div>
          <div className="flex gap-3">
            <button 
              onClick={exportCSV}
              className="flex items-center gap-2 px-5 py-3 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50 transition-all active:scale-95 text-sm"
            >
              <Download size={18} />
              CSV
            </button>
            <button 
              onClick={exportPDF}
              className="flex items-center gap-2 px-5 py-3 bg-secondary text-white font-bold rounded-xl hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-secondary/10 text-sm"
            >
              <FileText size={18} />
              PDF Report
            </button>
        ...
        <div ref={reportRef} className="space-y-8">
          ...
          <div className="bento-card p-10">
            <div className="flex items-center justify-between mb-10">
              <div>
                <h3 className="text-xl font-bold text-secondary">Comparativo Mensal</h3>
                <p className="text-xs text-slate-400 mt-1">Fluxo de caixa nos últimos 6 meses</p>
              </div>
              ...
              <div className="h-80 w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyData} barGap={8}>
                    ...
                    <Bar name="Entradas" dataKey="receitas" fill="#10B981" radius={[8,8,0,0]} barSize={24} />
                    <Bar name="Saídas" dataKey="despesas" fill="#fb7185" radius={[8,8,0,0]} barSize={24} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bento-card p-10">
              <h3 className="text-xl font-bold text-secondary mb-1">Concentração de Gastos</h3>
              <p className="text-xs text-slate-400 mb-10">Análise proporcional por categoria</p>
              
              {categoryData.length === 0 ? (
                <div className="flex items-center justify-center h-48 text-slate-400 text-sm">Sem despesas registadas.</div>
              ) : (
                <div className="h-72 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={categoryData} innerRadius={80} outerRadius={110} paddingAngle={6} dataKey="value" stroke="none">
            ...
            <div className="bento-card p-10">
              <div className="flex items-center justify-between mb-8">
                <div>
                  <h3 className="text-xl font-bold text-secondary">Performance de Gastos</h3>
                  <p className="text-xs text-slate-400 mt-1">Categorias com maior volume financeiro</p>
                </div>
